...
def form(request):
    ...
    data['form'] = Telefone_organizadora()
    return render(request, 'form.html', data)

def create(request):
    form = OrganizadoraEvento(request.POST or None)
    form = Telefone_organizadora(request.POST or None)
    if form.is_valid():
        form.save()
...
def view(request, pk):
    ...
    data['db'] = Telefone.objects.get(pk=pk)
...
def edit(request, pk):
    ...
    data['db'] = Telefone.objects.get(pk=pk)
    data['form'] = Telefone_organizadora(instance=data['db'])
    return render(request, 'form.html', data)
...
def update(request, pk):
    ...
    data['db'] = Telefone.objects.get(pk=pk)
    form = Telefone_organizadora(request.POST or None, instance=data['db'])
    if form.is_valid():
        form.save()
    return redirect('home')
